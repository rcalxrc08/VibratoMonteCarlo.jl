...
@show result = vibrato(bs, rfCurve, mc, EUData, VibratoMonteCarloStandard(Nsim2));
...
@show result = vibrato(hest, rfCurve, mc, EUData, VibratoMonteCarloStandard(Nsim2));
@show EuPrice = pricer(hest, rfCurve, mc, EUData);

vg = VarianceGammaProcess(sigma, 0.1, 0.01, Underlying(S0, d));
@show result = vibrato_saltando(vg, rfCurve, mc, EUData, VibratoMonteCarloStandard(Nsim2));
